...
In the field of sports analytics, experts, often statisticians or data scientists, use data and quantitative methods to evaluate player performance, analyze player safety, predict outcomes of games, set betting odds, and develop in-game strategies, among other things.
Sports teams and organizations also use analysis to make business decisions, while the public can use sports analytics to play fantasy sports, make bets, or provide evidence to help win arguments about whose favorite team is better.


On account of the fact that American football is the most popular sport in the U.S., in this paper, we will analyze the performance of NFL teams.
...
Each team belongs to either the AFC (American Football Conference) or the NFC (National Football Conference), and there are 16 teams in each conference.
...
**Note:** The data set covers the years 2000 through 2019. In that span, the Houston Texans were added as a team (2002), and there have been multiple teams that changed locations.
...
After the end of the regular season, six teams from each conference (four division winners in each conference and two wild card teams) advanced to the playoffs.
...
This added an extra wild card round game between the 7 seed and the 2 seed.
...
The data we are using in this project comes in the format of a .csv file, containing football games played by NFL teams from year 2000-2019.
This data originally contained 19 variables related to NFL game scores, outcomes, and statistics.
...
To derive a categorical variable, a loop was utilized to replace values of "NA" with a string "0" in the tie column and replace team names with a string "1" in this same column.
This also helped to clean the data, since it doesn'tt make sense to list a team winner for a game that ended with a tie (as the original data did).
After this loop, the strings of "0" and "1" were converted to a categorical variable.
This process created a categorical variable with a value of 0 if an NFL game ended with one team winning and a value of 1 if an NFL game ended in a tie.
...
In this section, we use various methods to explore the data set.
First, we ran a general summary of the data.
This gave us information about each variable in the data set.
For numeric variables, we received the minimum and maximum values, as well as the median, mean, lower quartile, and upper quartile.
For the categorical variable, we received counts for the number of times each factor occurs in the data.
This also gave us information about each variable's type.
For specific information about the values of numeric variables, please refer to the table below.
...
| Variable Name | Minimum | Lower Quartile | Median | Mean  | Upper Quartile | Maximum|
|---------------|---------|----------------|--------|-------|---------------|---------|
| pts_win       | 3       | 21             | 27     | 27.78 | 34             | 62     |
| pts_loss      | 0       | 10             | 16     | 16.09 | 21             | 51     |
| yds_win       | 47      | 308            | 361    | 361.6 | 415            | 653    |
|yds_loss       | 26      | 251            | 306    | 309.1 | 366            | 613    |
|turnovers_win  | 0       | 0              | 1      | 1.08  | 2              | 7      |
|turnovers_loss | 0       | 1              | 2      | 2.168 | 3              | 8      |

We also decided to skim the data set.
This provided us with information about the dimensions of our data set, along with the frequency of the various data types.
For character variable types, this gave us information about the minimum and maximum lengths of characters, the number of NA values in these variables, and the number of unique characters in each of these variables.
Skimming also produced information about the time variable and factor variable in the data.
We received the frequency for each of the factors in this variable, and we received various information about the time variable as well.
Similarly to using the summary function, utilizing the skim function also gave us the 5 number summary for the numeric variables.
The only differences between the summaries is that skimming gave us the standard deviations for each of the numeric variables and produced histograms as well.
The results from skimming the data are included below.
...
While the summaries are good sources of information about the data, visual summaries can also helpful in recognizing patterns and trends in the data.
Here, we provide boxplots for each of the numeric variables (excluding year which was read in as a numeric variable).
The top row shows a boxplot of the winning team's points scored, a boxplot of the losing team's points scored, and a boxplot of the winning team's yards gained.
The bottom row shows a boxplot of the losing team's yards, a boxplot of the winning team's turnovers, and a boxplot of the losing team's turnovers.
The boxplots generally show what we would expect.
Winning teams tend to score more points, gain more yards, and commit fewer turnovers than the losing team.
It is interesting to see that a some teams turned the ball over six and seven times and still won the game.

```{r, echo = FALSE, warning = FALSE, message = FALSE}

plot1 <- ggplot(footballgames, aes(y = pts_win)) + 
  geom_boxplot() + 
  ggtitle("Winner's Points") +
  ylab("Number of Points Scored")

plot2 <- ggplot(footballgames, aes(y = pts_loss)) + 
  geom_boxplot() + 
  ggtitle("Loser's Points") +
  ylab("Number of Points Scored")

plot3 <- ggplot(footballgames, aes(y = yds_win)) + 
  geom_boxplot() + 
  ggtitle("Winner's Yards") +
  ylab("Number of Yards")

plot4 <- ggplot(footballgames, aes(y = yds_loss)) + 
  geom_boxplot() + 
  ggtitle("Loser's Yards") +
  ylab("Number of Yards")

plot5 <- ggplot(footballgames, aes(y = turnovers_win)) + 
  geom_boxplot() + 
  ggtitle("Winner's Turnovers") +
  ylab("Number of Turnovers")

plot6 <- ggplot(footballgames, aes(y = turnovers_loss)) + 
  geom_boxplot() + 
  ggtitle("Loser's Turnovers") +
  ylab("Number of Turnovers")

plot_grid(plot1,plot2,plot3,plot4,plot5,plot6)
```

Also shown are histograms of the numeric variables. 
This is useful in identifying the most common trends in NFL games.
The top left and top right histograms show the winning team's points scored and the losing team's points scored.
Based on the histograms, the winning team usually scores between 20 and 35 points in a game, while the losing team usually scores between 5 and 25 points in a game.
It's interesting to note that one of the most frequent amount of points scored for losing teams is within the 5-10 point range, while this is exceptionally rare for the winning team.
This suggests that there aren't many NFL games in which both teams score few points (although the 2022 Broncos are trying hard to increase this number).
The middle histograms show the winning team's yards gained and the losing team's yards gained.
Based on the histograms, it appears the winning team typically gains 300-400 yards while the losing team typically gains 250-350 yards.
The histograms on the bottom show the number of turnovers committed by the winning team and the number of turnovers committed by the losing team, respectively.
It appears that winning teams often commit one or fewer turnovers per game while losing teams often commit one or two turnovers per game.
...
In addition to boxplots and histograms, we also decided to display density curves for the numeric variables.
There are some interesting density curves shown.
The densities for the winning team's yards gained and the losing team's yards gained don't show any gross departures from normality, which isn't in line with what we expected.
The densities for the number of turnovers committed by the winning and losing teams both provide some interesting curves as well.
...
#### Which Team won the Super Bowl the most from 2000-2019?

In this section, we can check if there is any team that won the Super Bowl multiple times over the 20-year range.
We decided to create a data frame consisting of Super Bowl winners and display the corresponding number of wins.
We then drew a bar plot to help visualize the number of wins.
We found out that the New England Patriots won the most with 6 games won.
Next up, we found that there are three teams that won the same amount of times (2): the Baltimore Ravens, the New York Giants, and the Pittsburgh Steelers.
Unfortunately, other teams that have won the Super Bowl from 2000-2019 only won once. 
...
The National Football League (NFL) is a composition of two conferences, the AFC (American Football Conference) and the NFC (National Football Conference). 
Here, the objective is to find out which conference won the most games (Home and Away) whenever an AFC team meets with an NFC team. 
Additionally, we want to see the distribution of points scored by each conference in these games. 
In order to do this, two new columns, 'HT' and 'AT', are created to determine if the home and away team was in the AFC or the NFC, with reference to the list of AFC and NFC teams, as well as the 'home_team' and 'away_team' variables.
However, there were games that were played between two AFC teams or  two NFC teams which we are not interested in. Also, the Super Bowl games are played in a neutral venue, so it would not make sense to have them included here. 
Then the data was filtered out for only the matches between AFC and NFC teams.
There are 5324 matches in the dataset. 
Out of these 5324 games, only 1288 were played between an AFC team and an NFC team. 
The respective number of wins for each conference is then calculated, and a bar plot of the same is created.
Further, a box plots of points scored for each conference are created.
...
plot_afcvsnfc_total_win<-afcvsnfc %>%
  ...
  ggtitle("Number of Games Won by Conference Playing Home and Away ") +
...
Out of 1288 matches played between an AFC team and an NFC team, the AFC team won 661 games versus the NFC's 623. Only four of the matches ended in a tie. 
The plot below shows the distribution of points scored by the winning conference's team when the AFC and NFC played each other. 

```{r,echo = FALSE, warning=FALSE, message = FALSE, results = 'hide', fig.keep = 'all'}
pts_afcvsnfc<- ggplot(filter(afcvsnfc, winner!= "Tie"))+geom_boxplot(aes(x=win_conf,y=pts_win, fill= win_conf))+xlab("Winning Conference")+ylab("Points Scored")+scale_y_continuous(limits = c(0, 60))+ggtitle("Points Won by AFC Vs NFC Conference Teams at Home and Away.")+ guides(fill=guide_legend(title="Win Conf."))
...
From the above boxplots, we can see that the distribution of points scored by the winning conference's team when the AFC and NFC played is similar. 
The mean points scored by the AFC's team when they played an NFC team is 28.6 and the mean points scored by the NFC's team when they played an AFC team is 28.1. 

#### What does the Number of Wins at Home look like for AFC vs NFC Teams?

Here, we want to see who wins more games at home, the NFC's teams or the AFC's teams.
In order to achieve this, first NFC teams' home games are filtered out from the original data set.
It is to be noted that Super Bowls are not included, since they are played in a neutral venue, and matches between two teams from same conference are also excluded. 
In this new dataset, a new column called 'win_conf' is created.
This stores the result of the winning team's conference. 
Then, the number of games won by each conference's teams whenever the NFC played at home is calculated. 
Similar steps are carried out whenever the AFC teams were playing at home. 
Finally, side-by-side bar plots are created for comparison.
...
From the plot above, we can see that out of 557 games played at an NFC team's home, NFC teams won 312 matches versus the visiting AFC teams' 245 wins.
Similarly, out of 556 games played at an AFC team's home, AFC teams won 331 matches versus the visiting NFC teams' 222 wins. 
Overall, AFC teams are winning more home games compared to the NFC.

#### Comparison of Points for AFC and NFC Teams Playing each other.

Here, the objective is to compare the distribution of points scored for an AFC team vs an NFC team, playing in their respective homes. 
In order to do this, a data set that contains only games played at the home of NFC teams (with visiting AFC teams) is used, using 'homewin_nfc' data. 
The Super Bowl games and the games that are 'tied' are not included in the analyses.
Then, a box plot of the variable 'pts_win' is created based on 'win_conf' for comparison. 
Similar analyses are carried out for games played at the home of AFC teams (with NFC teams visiting), using 'homewin_afc' data. 
Lastly, the two separate plots created are displayed in the same panel for side-by-side comparison.

```{r,echo = FALSE, warning = FALSE}
plot_pts_nfc_home<-ggplot(data=homewin_nfc, aes(x=win_conf,y=pts_win, fill= win_conf))+geom_boxplot() +stat_summary(fun.y=mean, geom="point", shape=20, size=3, color="purple", fill="purple")+xlab("Winning Conference")+ylab("Points Won")+scale_y_continuous(limits = c(0, 60))+ggtitle("Points Scored at NFC's Home.")+ guides(fill=guide_legend(title="Win Conf."))

ggplotly(plot_pts_nfc_home)

plot_pts_afc_home<-filter(homewin_afc, winner != "Tie") %>% ggplot(aes(x=win_conf, y=pts_win, fill= win_conf))+ geom_boxplot()+stat_summary(fun.y=mean, geom="point", shape=20, size=3, color="purple", fill="purple")+xlab("Winning Conference")+ylab("Points Won")+scale_y_continuous(limits = c(0, 60))+ggtitle("Points Scored at AFC's Home.")+ guides(fill=guide_legend(title="Win Conf."))
...
The above plot shows the distribution of points scored by the winning conference's teams at each of the conference's team homes.
The mean, median, and first and third quartiles all look similar in both of the plots.
Whenever the game was played at the NFC team's home, the NFC scored 28.45 points on average, compared to the AFC's 28.42.
The maximum number of points won by an AFC team playing at an NFC team's home is 55 compared to 57 for the NFC team.
Similarly, when the games were played on an AFC team's home turf, the average points scored by the NFC team is 28 vs the AFC's 28.7. 
On one occassion, one AFC team scored 56 points, which is the maximum points scored at an AFC team's home. 
The maximum points scored by an NFC team at an AFC team's home is 51, and the minimum is 9. 

#### Which Conference Won the most Super Bowls?

Super Bowl games are played between the two best teams from each conference.
Here, the objective is to see the number of Super Bowls won by each conference between 2000 and 2019. 
For this, a subset of original data where 'week' is 'Superbowl' is filtered out.
Then, the winning conference for each of these games is identified, with reference to the list 'afc_teams' and 'nfc_teams' and stored in a new variable 'win_conf'. 
Finally, a bar plot is created for the number of Super Bowls won by each conference.
...
Out of the 20 Super Bowl matches, the AFC won on 13 occassions, compared to the NFC's 7 wins. 
...
Since there were 5,304 games played, the home team has won 3,025 of 5,304 games for a percentage of 57%.
...
There are different questions that we tried to address in our analyses as a group, leading to a multitude of inferences we can draw from our data set. 
Several takeaways in our findings are summarized below. 

Focusing on the Super Bowl, we found that it was primarily won by the New England Patriots with a whopping record of 6 wins within 2000-2019.
We also found that between 2000 and 2019, AFC teams have won 6 more Super Bowls compared to the NFC.  

Regarding the comparison between AFC and NFC teams, AFC teams have won more matches when playing against NFC teams either home or away, with only 4 games ending in a tie.
There is not much difference in the distribution of points won between the two conferences in these matches based on the summary statistics.
Among the games played at an NFC team's home, the NFC won more games compared to the AFC. 
Similarly, AFC teams were winning more games against the NFC whenever the AFC's teams were playing in their home.
However, based on the number of home games played between the two conferences, the AFC has a better home record. 
There is not much difference in the distribution of points scored by the winning conferences playing at both the NFC team's home and the AFC team's home. 

Another primary finding in this study was that the home team has an obvious advantage.
The bar plot comparing the number of football games won by the home team and the number of games won by the visiting team showed that the home team wins more often than the visiting team.
In addition, the boxplots comparing the yards gained and points scored between home and away teams revealed that the home team gains more yards and scores more points than away teams.
The two sample t-tests revealed that the difference in mean yards gained between home and away teams and the difference in mean points scored between home and away teams were both statistically significant, at a 5% significance level.
These differences were much smaller for the number of turnovers committed by home teams compared to away teams.
This indicates that playing at home probably doesn't have a large effect in causing the away team to turn the ball over more often.

Lastly, we decided to analyze the AFC West division, where we observed their winning percentages every year from 2000-2019. 
Our analysis found that out of the four teams, it is difficult to spot patterns or trends in their winning percentages just from simply looking at the plots of these four teams' winning percentages. 
We also attempted to fit multiple time series models, using them to forecast winning percentages three years into the future.
However, our projections tended to underestimate, compared to the actual results that we have available for the 2020 and 2021 seasons.
Thus, we generally found that winning percentages usually fluctuate over long periods of time among the AFC West teams.
In terms of time series modeling, we think including explanatory variables into these models would improve the overall fits, as well as improve the forecast accuracy.
One highlight that we thought was noteworthy to mention was the fact that the Chiefs have been on a strong streak within the last seven years as shown by our data. 
This impressive achievement was even supplemented due to the fact that they won Super Bowl in 2020.
...
([source](https://phermanto2.shinyapps.io/project-we-r-pab/))
([source](https://www.espn.com/nfl/standings/_/season/2020))
([source](https://en.wikipedia.org/wiki/List_of_Super_Bowl_champions))
