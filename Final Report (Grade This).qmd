...
We decided to create a data frame consisting of the winners of the Super Bowl and display the corresponding number of wins.
We then drew a bar plot to help visualize the number of wins.
We found out that New England Patriots won the most with 6 games won.
Next up, we found that there are three teams that won the same amount of times (2): Baltimore Ravens, New York Giants, and Pittsburgh Steelers.
...
Here, the objective is to find out who have won most games (Home and Away) whenever an AFC team meets with a NFC team. Additionally, to see the distribution of the points won by each conference. 
...
However, there were games that was played between two AFC teams as well as two NFC teams which we are not interested in. Also, the Superbowl games are played in the neutral venue, so it would not make sense to have them included here. 
Then,the data was filter out for only the matches between AFC and NFC teams. There are 5324 matches in the dataset. 
Out of 5324 games, only 1288 matches were played between an AFC team and NFC team either home or away. 
The respective number of wins for each conference is then calculated and a bar plot of the same is created.Further, a box plot of distribution of points won for each conference is created.
...
Out of 1288 matches played between an AFC team and an NFC team, AFC team won 661 games versus NFC's 623. Only, 4 of the matches ended in a tie. 
The plot below shows the distribution of points won by the winning conference's team when AFC and NFC played each other. 
```{r,echo = FALSE, warning=FALSE, message = FALSE}
...
From the above box-plots, we can see that the distribution of points won by the winning conference's team when AFC and NFC played is almost similar. 
The mean points won by AFC's team when they played NFC's teams is 28.6 and the mean points won by NFC's team when they played AFC's teams is 28.1. 
...
From the plot above we can see that, out of 557 games played at NFC's teams home, NFC teams won 312 matches versus AFC's teams 245 wins.
Similarly, out of 556 games played at AFC's teams home, AFC teams won 331 matches versus NFC's teams 222 wins. 
Overall, AFC's teams are winning more home games compared to NFC's.
